...
export async function serverLog({
  ...
  try {
    ...
    await db.execute({
      sql: 'INSERT INTO server_logs (level, category, message, details, ip_address, user_agent, request_path) VALUES (?, ?, ?, ?, ?, ?, ?)',
      args: [level, category, message, details ?? null, ip_address, user_agent, request_path],
    });
  } catch {
